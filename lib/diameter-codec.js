...
var decodeAvp = function(buffer, start, appId) {
    ...
    var avpTag = dictionary.getAvpByCode(avp.codeInt, undefined);
    if (avpTag == null) {
        throw new Error('Unable to find AVP for code ' + avp.codeInt + ' for app ' + appId);
    }
    avp.code = avpTag.name;

    var hasVendorId = avp.flags.vendor && avpTag.flags.vendorBit;
    if (hasVendorId) {
...
var encodeAvp = function(avp, appId) {
    var avpTag;
    if (!_.isNumber(avp[0])) {
        avpTag = dictionary.getAvpByName(avp[0], avp[2]);
    } else {
        avpTag = dictionary.getAvpByCode(avp[0], avp[2]);
    }
    if (avpTag == null) {
